...
记录一下下标即可，建表时间复杂度$O(n\log n)$，查询$O(1)$。这里以CF1879F为例，该题需要维护每个值域内的最大和次大$h$，并且进行查询

```cpp
struct node{
    pii mx,sx;//每个区间的最大值和次大值
}st[19][N];
...
如下表示$\left\lfloor\frac{n}{i}\right\rfloor$的所有取值可能集合，其中$|D(n)|=\lfloor\sqrt{4n+1}\rfloor -1$
$$
D(n)=\left\{\left\lfloor\frac{n}{i}\right\rfloor : 1\le i\le n,\ i\in\mathbb{N}_+\right\}.
$$
对于$d\in D(n)$，所有满足$\lfloor \frac{n}{i}\rfloor=d$的整数$i$的取值范围为
$$
\lfloor \frac{n}{d+1}\rfloor+1\le i\le \lfloor \frac{n}{d}\rfloor
$$
计算时只需让$l=1$，然后通过$\lfloor \frac{n}{l}\rfloor=d,r=\lfloor \frac{n}{d}\rfloor$计算出$r$，如果有函数就进行前缀和处理或者其他方式快速计算。分块的时间复杂度为$O(\sqrt{n})$

#### 上取整转下取整

由$\lceil\frac{n}{i}\rceil=\lfloor\frac{n-1}{i}\rfloor+1$​，有如下变形
$$
\sum_{i=1}^{n} f(i)g\left(\left\lceil \frac{n}{i}\right\rceil \right)
=
f(n)g(1)+\sum_{i=1}^{n-1}f(i)g\left(\left\lfloor\frac{n-1}{i}\right\rfloor+1\right)
$$
...
生成函数的每一个系数可以认为是这一个物品有多少种选择方式

$A(x)$逆元满足$A(x)B(x)=1$，逆元存在的条件为$[x^0]A(x)\ne 0$。

#### 常见的逆

$A(x)=1+x+x^2+\cdots$ 和 $B(x)=1-x$，即$\frac{1}{1-x}=\sum_{i\geq 0}x^i$

$A(x)=1+ax+a^2x^2+\cdots$ 和 $B(x)=1-ax$，即$\frac{1}{1-ax}=\sum_{i\geq 0}a^ix^i$

$A(x)=\binom{k-1}{0}+\binom{k}{1}x+\binom{k+1}{2}x^2+\binom{k+2}{3}x^3+\cdots$和$B(x)=(1-x)^k$
...
$\frac{1}{1-f(x)}=1+f(x)+f^2(x)+f^3(x)+...$
...
$$
(x+y)^\alpha=\sum_{k=0}^{+\infty} \binom{\alpha}{k}x^{\alpha-k}y^k,\binom{r}{k}=\frac{r(r-1)...r(r-k+1)}{k!}
$$
...
下面两个模板源点和汇点用$n+m+1,n+m+2$这种的

### SSP模板(Dinic实现)

设网络最大流量为$f$，那么最坏复杂度$O(nmf)$。关于点和边的复杂度为$O(n^32^{n/2})$
...
解决了卡SPFA的问题，每轮$O(E\log V)$，总共时间复杂度$O(FE\log V),F$为最大流量
...
以下概念基于1base字符串，0base简单换算即可

反串$R(s)$：字符串$S[1,i]$反串为$R(s)=S[i,1]$

回文串：$S=R(S)$的字符串

回文中心：

* 奇长度回文串，回文中心为$S\lceil\frac{n+1}{2}\rceil$
* 偶长度回文串，回文中心为$S\lceil\frac{n}{2}\rceil$与$S\lceil\frac{n}{2}+1\rceil$中间，如$abc|cba$

回文半径$L$：回文中心到回文串的左右端点距离相等，此距离为回文半径。abcba半径为$3$，$abcddcba$半径为$4$
...
回文串和border：对于回文串$S$​，回文串的回文前（后）缀等价于一个Border

$t$是$s$的border（$|s|\le 2|t|$），$s$是回文串当且仅当$t$​是回文串

$t$是回文串$s$的border，则$|s|-|t|$是$s$的周期。$|s|-|t|$为$s$的最小周期，当且仅当$t$时$s$​的最长回文真后缀。
...
![引理图示](D:\ACM\板子\图片\回文自动机\引理图示.png)

$s$的所有回文后缀按照长度排序后，可以划分成$\log |s|$段等差数列



在相邻字符以及首尾当中插入字符#之后，所有极长回文串一定为奇数。

插入#的回文串长度记为$|S^{\#}|$，对应原始回文串长度有$|S|$，那么有$|S^{\#}|=2|S|+1,|S|=\frac{|S^\#|-1}{2}=\lfloor\frac{|S^\#|}{2}\rfloor$，对于回文半径同样适用。 

最右回文串：所有已经求的回文串当中，右端点最靠右的一个。

$Len[i]$表示以$i$​为回文中心的最大回文半径。

**弱周期引理：** 若$p,q$都是$S$的周期并且$p+q\le |S| $，则$\gcd(p,q)$也是$S$的周期

**周期引理**：若$p,q$都是$S$的周期并且$p+q-\gcd(p,q)\le |S| $，则$\gcd(p,q)$也是$S$的周期

### 模板

流程：如果当前的位置已经在最右回文串的右侧了，就暴力匹配自己的回文串。如果不是，但是自己加上自己对称位置的回文串的回文半径小于等于有边界的话，不匹配;如果超过，就暴力匹配
...
void Manacher(const string&s,vector<int>&d1,vector<int> &d2){
    ...
    d1.resize(n);
    d2.resize(n);
    //l,r分别表示最右回文串的左右端点（恰好属于回文串边界）
    //d1奇数长度回文半径(含上中心是奇数，不含是偶数),d2是把自己的一半偏右为中心，半径是一半 
    //回文串长度2*d1[i]-1,2*d2[i],区间[i-d1[i]+1,i+d1[i]-1],[i-d2[i],i+d2[i]-1]
    for(int i=0,l=0,r=-1;i<n;i++){
        int k=(i>r)?1:min(d1[l+r-i],r-i+1);//找到对称点位置
    ...
    for(int i=0,l=0,r=-1;i<n;i++){
        int k=(i>r)?0:min(d2[l+r-i+1],r-i+1);//找到对称点j的右边的j+1位置,还不明白的话，画出来一个回文串左右两侧都是相邻的还需要对称就知道是何意味了
        while(i-k-1>=0&&i+k<n&&s[i-k-1]==s[i+k]) k++;
        d2[i]=k--;//换算成长度-1,也就是真实的偏移量
        if(i+k>r){
            l=i-k-1,r=i+k;
        }
    }
...
本质不同回文串一定出现在manacher的暴力匹配的时候，即每次最右端点进行右移的时候，最多右移$n$次，所以说最多有$n$个本质不同的回文子串。每次扩展的时候放进去去重即可得到所有本质不同的回文串。
...
有的时候需要自底向上的做dp，本质是拓扑序从大到小进行计算，SAM的拓扑序可以用按照长度进行基数排序解决，有如下代码：
...
对S建SAM,考虑枚举T的每个前缀，如果是公共子串，那么一定是T前缀的某个后缀，并且可以沿着后缀自动机找到。然后求出每个前缀的最长后缀是公共子串的最大值就是答案。考虑状态转移，上一次已经匹配到了一个状态，那么检测这个状态是否有新加入字符的转移边，有就$+1$，并且走向这个转移边指向的状态，反之就继续去找它的后缀，也就是跳后缀链接link并且缩短长度为对应节点的最长长度。到了根节点都没有找到就是$0$，或者就是找到了$+1$。时间复杂度$O(n)$，因为最多添加$n$次状态，那么也就是最多跳$n$​次后缀链接link。
...
把每个字符串用唯一不同的标识符连接起来，那么一个字符串是子串当且仅当其可以只走一些非标识符字符达到某个标识符。按长度从大到小进行dp，设$f(s)$表示状态$s$下有哪些字符可以通过只走非标识字符达到标识字符，然后状压第$i$位$1$表示具有第$i$个标识符。那么很显然是它的若干后继节点的$f$的并集。如果后继节点有标识符节点的话，也是需要进行合并的。时间复杂度$O(|\sum||S|)$​

```cpp
for(int i=S.tot-1;i>=0;i--){//tt表示有多少个字符串
    int u=ord[i];
...
选最短字符串建自动机，仿照双串的思路，遍历每一个其他字符串和这个自动机的LCS，但是这里我们记录一下每次匹配成功之后，某一个节点能够取到的最大值。更新某一个字符串下该节点的最长子串长度，同时顺着他的后缀链接link向上跳的所有节点也是应该被更新的，因为都是其后缀。我们先进行单点的更新，然后自底向上遍历一次SAM进行整个自动机的更新。然后我们还需要记录全局每个节点的最长公共前缀，那么就是当前的和先前的取min，因为要保证其一定是所有字符串的公共子串，所以说保留每个等价类子串里面每个字符串能匹配到的最大值的最小值。时间复杂度$O(\sum|S_i|)$，因为拿最短子串建自动机遍历字符串个数次自动机一定小于等于所有字符串数量。
...
SAM定义就是分成了若干互无交集的子串等价类，并且每一个等价类内部子串长度连续。那么一个等价类内部有$len(u)-len(link(u))$个本质不同子串。遍历每一个非$0$​节点即可。

##### 法二（SAM上DP）

因为任意一个子串都可以通过$t_0$遍历得到并且不会有重复的路径，所以子串个数等价于路径数量。从大到小计算出$f(u)$表示节点$u$沿着转移边有多少条路径，那么就是自己所有能转移状态路径数加上只选自己一个点数量，即$f(u)=1+\sum_{v=tran(u,c)}f(v)$

#### 本质不同公共子串个数

对串$S$建自动机，所有公共子串一定是某个前缀的后缀，和之前求最长公共子串一样遍历每个前缀找到最长后缀是公共子串。那么它到根节点路径上一定全是合法的，标记每个节点的最长的作为公共子串的长度。那么答案就是每个节点能取到的最长长度，即$\sum{\max(0,mx(u)-len(link(u)))}$，时间复杂度$O(|S|+|T|)$​

#### 两子串本质不同子串并集大小

由容斥原理转化为两个字符串各自本质不同子串个数-两个子串的本质不同公共子串个数。易求。

#### 字典序第$k$大子串

因为从根开始沿着某一个路径可以唯一确定一个子串，并且这个节点之后的路径与前缀无关。所以说找到第$k$大的子串等价于找到第$k$大的路径，那么接下来需要记录的就是每个节点的路径的数量。考虑dp，设$f(u)$表示$u$节点及其之后对应状态所以可以接上的后缀的数量。那么有$f(u)=\sum_{(v,u)\in E}f(v)$，如果求的是本质不同子串的话，那么每个子串只有一种，还需要额外$+1$，表示有一种结束方式，如果允许重复的话，一个子串在字符串当中有$|endpos(u)|$个位置，这些位置都需要加上。至此转移完成。类似于线段树二分找第$k$大，每次$k>f[v]$的时候直接减去即可。每次来到一个节点如果小于该节点所有对应方式数值，那么直接输出。反之需要减去只有这个节点的贡献，如果没有任何一种合法的转移路径了，那么就是无解。时间复杂度$O(|\sum||S|)$​

## 回文自动机PAM

### 概念

用$S(u)$表示节点$u$代表的字符串，后继边$tran(u,ch)=v$表示节点$u$有后继边指向$v$节点，表示$\operatorname{ch}S(u)\operatorname{ch}=S(v),len(v)=len(u)+2$，失配边$fail[u]=v$表示$S(u)$的最长回文后缀是$S(v)$​

奇根长度为$-1$，偶根长度为$0$。

### 模板

空间复杂度$O(|S||\sum|)$，也可以采用哈希表或者map存边。建树的时间复杂度是$O(|S|)$

```cpp
struct Palindromic_AutoMaton{
    int num[maxn];//num表示每种前缀最长回文字符串的出现个数
    ...
    void build(){//类似后缀自动机上的操作，按拓扑序把每个点的值加到fail上面，统计每种回文子串的出现个数
        for(int i=tot;i>=2;i--){
            num[fail[i]]+=num[i];
        }
        num[0]=num[1]=0;
...
如果题目某一个参数过大（如转移轮数过大），并且每轮转移都遵循相同的转移方式的话，可以尝试构造转移矩阵并用矩阵快速幂进行解决。
...
有时候可能会卡unordered_map哈希冲突，可以用如下手写哈希函数解决，万一就卡过去了呢()
...
struct pair_hash {
    size_t operator()(const pll &p) const {
        ...
        uint64_t x = ((uint64_t)(uint32_t)p.first << 32)
                   | (uint32_t)p.second;

        return custom_hash::splitmix64(x + FIXED_RANDOM);
    }
};//pll用的hash
unordered_map<long long, int, custom_hash> mp;

mp.max_load_factor(0.7);
mp.reserve(n);//减少大量触发rehash，稍微开大一点用多少留多少
```
